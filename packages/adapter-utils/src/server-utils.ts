...
export const DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE = [
  ...
  "- When a Paperclip task workspace helper is available, use the approved `task_workspace.py` commands for all workspace reads, searches, lists, extracts, and writes. Example recovery reads: `task_workspace.py read review_state.md` and `task_workspace.py read page-4.txt`. Use `task_workspace.py search`, `task_workspace.py list`, and `task_workspace.py write` for the matching operations. For multi-line writes, pass content on stdin to `task_workspace.py write <file>` or use `task_workspace.py write-base64 <file> <base64-content>`; do not pass the content as an extra shell argument. Do not use plain shell file commands such as `cat`, `grep`, `sed`, `ls`, `printf`, `echo`, `base64`, pipes, or shell redirection for task workspace files if command policy restricts the workspace to that helper.",
...
export function renderPaperclipWakePrompt(
  ...
  if (artifactInventory) {
    ...
    lines.push(
      "Use approved task workspace helper commands such as `task_workspace.py read`, `task_workspace.py search`, `task_workspace.py list`, `task_workspace.py write`, `task_workspace.py write-base64`, and `task_workspace.py extract-pdf` for workspace files. For recovery files, read `review_state.md` with `task_workspace.py read review_state.md` and read `page-4.txt` with `task_workspace.py read page-4.txt`; for multi-line writes, pass content on stdin to `task_workspace.py write <file>` or use `task_workspace.py write-base64 <file> <base64-content>`, never as an extra shell argument. Do not try plain `cat`, shell `grep`, `sed`, `ls`, `printf`, `echo`, `base64`, pipes, or shell redirection when the task workspace is under command policy.",
    );
    lines.push(
